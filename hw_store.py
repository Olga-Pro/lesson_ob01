...
class Store():
    ...
    def add_item(self, new_item):
        ...
        print(f"Добавлен товар {new_item}")

    def del_item(self, key):
        if self.items.get(key, None):
            cur_item = self.items.pop(key)
            print(f"Удален товар {key} по цене {cur_item}")
        else:
    ...
    def upd_price(self, key, price):
        #обновление цены
        #можно использовать add_item и для модификации тоже
        if price > 0:
            self.items.update({key: price})
            print(f"Изменена цена товара {key}. Текущая цена = {price}")
        else:
            print(f"Неверная цена товара {key}. Вы указали цену = {price}. Текущая цена = {self.items.get(key, None)}")
...
print(f"Магазин {shop02.name}")
shop02.add_item({"Яблоки": 110})
shop02.add_item({"Томаты": 190})
shop02.add_item({"Огурцы": 145})
shop02.add_item({"Морковь": 30})
shop02.add_item({"Свекла": 20})
shop02.all_items()
shop02.add_item({"Огурцы": 140})
shop02.upd_price("Огурцы", 0)
shop02.info_price("Яйца")
shop02.del_item("Томаты")

print(f"Магазин {shop03.name}")
shop03.add_item({"Сметана": 90})
shop03.add_item({"Хлеб": 40})
shop03.add_item({"Масло": 185})
shop03.add_item({"Морковь": 30})
shop03.add_item({"Свекла": 40})
shop03.all_items()
shop03.add_item({"Огурцы": 160})
shop03.upd_price("Огурцы", 0)
shop03.info_price("Яйца")
shop03.del_item("Томаты")
shop03.all_items()